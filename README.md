...
Reusing Derived Intermediate Operator State*. The prototype is built inside a
larger research storage engine, which is why the repository carries that
engine's name; everything specific to MVHT is listed below.
...
The development notes for the underlying storage engine (buffer pool,
Foster B-tree, profiling setup) are in [`README_ENGINE.md`](README_ENGINE.md).
They are unrelated to MVHT.
