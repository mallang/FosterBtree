...
DEFAULT_NUM_TRANSACTIONS = 2  # coverage logs: number of TX
...
def main():
    ...
    parser.add_argument("-mintx", "--min_coverage_fraction", type=float,
                        ...
                        help=(f"Minimum coverage fraction (default: {DEFAULT_MIN_COVERAGE})."
                              " If coverage fraction > 1.0 => over 100%."))
    ...
    print(f"table_0 has {row_count} rows.")
    
    # 2) build coverage fraction range from user config
    coverage_fractions = []
    frac = args.min_coverage_fraction
    while frac <= args.max_coverage_fraction + 1e-9:  # small epsilon
    ...
    for coverage_frac in coverage_fractions:
        coverage_count = int(round(coverage_frac * row_count))
        coverage_pct = coverage_frac * 100
        # total ops = coverage_count => average ops per TX = coverage_count / num_tx
        if args.num_transactions <= 0:
            num_tx = 1
        else:
            num_tx = args.num_transactions
        
        if coverage_count <= 0:
            # coverage fraction might be below 0.01 for large step => skip or do min?
        ...
        # out_filename = f"txs_u{coverage_frac:.3f}_{args.table0}"
        out_filename = f"txs_u{coverage_frac:.1f}_t0.csv"
        out_path = os.path.join(CSV_DIR, out_filename)
        print(f"\nCoverage fraction={coverage_frac:.3f} (~{coverage_pct:.1f}%), coverage_count={coverage_count}, => avg={approx_ops:.2f}, minop={minop}, maxop={maxop}.")
        print(f" => {out_filename} with {num_tx} TX(s).")
    ...
    # 3) call gen_txs_join.py for table_1..n => txs_join_t{i}.csv
