...
use super::{
    ...
    mvcc_hash_join_second_hash_table::SecondHashJoinTable,
...
pub const DEAFAULT_FIRST_BUCKET_NUM: usize = 256;
...
pub struct MvccHashJoinTable<T: MemPool> {
    ...
    num_buckets: usize,
    bucket_entries: Vec<Arc<SecondHashJoinTable<T>>>,
...
impl<T: MemPool> MvccHashJoinTable<T> {
    ...
    pub fn new_with_bucket_num(c_key: ContainerKey, mem_pool: Arc<T>, num_buckets: usize) -> Self {
        ...
        let mut bucket_entries: Vec<Arc<SecondHashJoinTable<T>>> = Vec::with_capacity(num_buckets);
        for i in 0..num_buckets {
            let second_table = SecondHashJoinTable::new(c_key, mem_pool.clone());
    ...
    pub fn insert(
        ...
        second_table.insert(&key, &pkey, &ts, &tx_id, &val)
    }
    ...
    pub fn update(
        ...
        second_table.update(&key, &pkey, &ts, &tx_id, &val)
        // let (recent_chain, history_chain) = &self.bucket_entries[index];
    ...
    pub fn delete(
        ...
        second_table.delete(&key, &pkey, &ts, &tx_id)
        // let (recent_chain, history_chain) = &self.bucket_entries[index];
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            if self.entry_index < self.current_entries.len() {
                ...
                let second_table = &self.table.bucket_entries[self.bucket_index];
                // Collect entries from both scanners
                let mut entries = Vec::new();

                loop {
                    if self.second_bucket_index >= second_table.num_buckets {
                        break;
                    }
                    let recent_chain = &second_table.get_recent_chain(self.second_bucket_index);
                    let history_chain = &second_table.get_history_chain(self.second_bucket_index);
                    ...
                    if let Some(ref mut scanner) = self.recent_scanner {
                        for (start_ts, end_ts, key, pkey, value) in scanner {
                            ...
                            if !self.seen_entries.contains(&key_pkey) {
                                ...
                                entries.push(MvccEntry::new(key, pkey, value, start_ts, end_ts));
                            }
                        }
                    }
