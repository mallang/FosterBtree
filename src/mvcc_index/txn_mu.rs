...
mod mvcctxn {
    ...
    impl MvccInner {
        ...
        pub fn new_txn<T: MvccIndex>(&self, inner: Arc<TxnMvccHashTable<T>>) -> Arc<Transaction<T>> {
    ...
    ///
    pub struct Transaction<T: MvccIndex> {
        pub(super) read_ts: u64,
        pub(super) txn_hash_table: Arc<TxnMvccHashTable<T>>,
        pub(super) local_storage: Mutex<HashMap<(T::Key, T::PKey), Delta<T::Value>>>,
        pub(super) committed: AtomicBool,
        pub(super) key_hashes: Mutex<(HashSet<u32>, HashSet<u32>)>,
    }

    impl<InnerIndex: MvccIndex> Transaction<InnerIndex> {
    ...
    impl<T: MvccIndex> Drop for Transaction<T> {
        fn drop(&mut self) {
...
pub struct TxnMvccHashTable<T: MvccIndex>{
    hash_table_inner: Arc<T>,
    mvcc: MvccInner,
}

impl<T: MvccIndex> TxnMvccHashTable<T> {
    pub fn inner_hash_table(&self) -> &T {
        &self.hash_table_inner
    }
    pub fn new(hash_table: &Arc<T>) -> Arc<Self> {
        Arc::new(Self {
    ...
    pub fn txn_new(self: &Arc<Self>) -> Transaction<T> {
...
mod tests {
    ...
    const _: () = {
        ...
        let _ = assert_send::<TxnMvccHashTable<MvccHashJoinTable<InMemPool>>>;

        fn assert_sync<T: Sync>() {}
        let _ = assert_sync::<TxnMvccHashTable<MvccHashJoinTable<InMemPool>>>;
    };
    ...
    fn test_txn_insert_into_hash_table() -> Result<()> {
        ...
        let hash_table_get_result = <MvccHashJoinTable<InMemPool> as MvccIndex>::get(&table_inner, &key, &pkey, committed_ts);
