...
mod slot {
    ...
    impl SlotMeta {
        ...
        pub fn check_match_remain_key_pkey(
            ...
            && &meta_bytes[4 + slot_remain_key_size as usize..4 + slot_remain_key_size as usize + slot_remain_pkey_size as usize] == remain_pkey
        }

        pub fn check_match_remain_key(
            slot: &Slot,
            meta_bytes: &[u8],
            key: &[u8],
        ) -> bool {
        ...
        pub fn get_remain_pkey<'a> (
            ...
            &meta_bytes[4 + slot_remain_key_size as usize .. (4 + slot_remain_key_size + slot_remain_pkey_size) as usize]
        }
...
mod record {
    ...
    use super::{slot::{SLOT_KEY_PREFIX_SIZE, SLOT_PKEY_PREFIX_SIZE}, DELETE_MARKER_IN_VAL_SIZE};
    ...
    impl LatestRecord {
        pub fn get_tx_id_and_next(bytes: &[u8]) -> (u64, u32) {
            let tx_id = u64::from_be_bytes(
                bytes[0..8].try_into().map_err(|_| "Failed to parse").unwrap()
            );
            let next_off = u32::from_be_bytes(
                bytes[8..12].try_into().map_err(|_| "Failed to parse").unwrap()
            );
            (tx_id, next_off)
        }
        ...
        pub fn to_bytes(
            ...
            let mut latest_bytes = Vec::<u8>::new();
            latest_bytes.extend(u64::to_be_bytes(tx_id));
    ...
    impl CommittedRecord {
        ...
        pub fn val_size_from_bytes(
            bytes: &[u8]
        ) -> Option<u32> {
            let res = u32::from_be_bytes(bytes[12..16].try_into().map_err(|_| "parse failed").unwrap());
...
use crate::{
    log_warn,
    mvcc_index::{hashtable_mu::hash_join_table_common::HashTableAccessMethodError, Delta, DeltaEntry, MvccEntry, Timestamp},
...
pub trait TableDataPageInterface: TableDataPageBase {
    ...
    fn insert_slot_and_meta(
        ...
        self.set_slot(new_slot_id, &new_slot);
    ...
    fn add_commit_version(
        ...
        let find_slot_res = self.find_slot_by_kpk(key, pkey);
        ...
        let slot = if let Some(sl) = find_slot_res {
            ...
            let need_space = Slot::space_need(key, pkey) + CommittedRecord::space_need_from_value(value, false);
            let free_space = self.free_space_without_compaction();
            if need_space > free_space {
                return Err(HashTableAccessMethodError::OutOfSpace);
            }
            let slot_id = self.insert_slot_and_meta(key, pkey);
            self.get_slot_ref(slot_id)
        };

        let latest_record_res = self.find_latest_record_before_ts_by_slot(slot, ts);
        let (latest_record_offset, prev_next_offset) = (latest_record_res.record_offset, latest_record_res.prev_next_offset);
    ...
    fn add_delete_version(
        ...
        let slot = if let Some(sl) = find_slot_res {
            ...
            let need_space = Slot::space_need(key, pkey) + CommittedRecord::space_need_from_value(&[], true);
            let free_space = self.free_space_without_compaction();
            if need_space > free_space {
                return Err(HashTableAccessMethodError::OutOfSpace);
            }
    ...
    fn get_value(
        ...
        let latest_record_res = self.find_latest_record_before_ts_by_slot(slot, ts);
        let (latest_record_offset, val_size) = (latest_record_res.record_offset, latest_record_res.val_size);
        if val_size.is_some() && latest_record_offset.is_some() {
            Some(self.read_bytes(latest_record_offset.unwrap() as usize + 8 + 4 + 4, val_size.unwrap() as usize).to_vec())
        } else {
            None
        }
    }
    ...
    fn find_latest_record_before_ts_by_slot(
        ...
        while latest_record_offset != 0 {
            let bytes = self.read_bytes(latest_record_offset as usize, std::mem::size_of::<u32>() + std::mem::size_of::<u64>() + std::mem::size_of::<u32>());
    ...
    fn next_version(&self, start_offset: u32) -> Option<CommittedRecord> {
        ...
        let get_record_fn = |off: u32| -> CommittedRecord {
            ...
            let val_size = CommittedRecord::val_size_from_bytes(bytes);
            let val =  {
                self.read_bytes(off as usize + 8 + 4 + 4, val_size.unwrap_or(0) as usize).to_vec()
            };
    ...
    fn rehash_export_to_new_page(
        ...
        let get_first_record_offset_fn = |slot: &Slot| {
            let meta_offset = slot.meta_offset();
            u32::from_be_bytes(self.read_bytes(meta_offset as usize, 4).try_into().map_err(|_| "failed parse").unwrap())
        };
        ...
        for (idx, slot) in slot_sli.iter().enumerate() {
            ...
            if is_new_page_fn(&key) {
                ...
                new_page.insert_slot_and_meta(&key, &pkey);
                ...
                while let Some(commit_record) = record {
                    if commit_record.is_deleted() {
                        free_space.push( 
                            ( offset, CommittedRecord::space_need_from_value(&commit_record.value, true) )
                        );
                        new_page.add_delete_version(&key, &pkey, commit_record.start_ts).unwrap();
                    } else {
                        free_space.push( 
                            ( offset, CommittedRecord::space_need_from_value(&commit_record.value, false) )
                        );
                        new_page.add_commit_version(&key, &pkey, commit_record.start_ts, &commit_record.value).unwrap();
                    }
    ...
    fn rehash_collect_used_space(
        ...
        let mut push_new_space_fn = |space: (u32, u32), slot_id: u32, slot_adrs: &mut SlotAddresses| {
        ...
        for slot_id in left_ids {
            ...
            let mut record = self.next_version(offset);
            while let Some(commit_record) = record {
                push_new_space_fn((offset, CommittedRecord::space_need_from_value(&commit_record.value, commit_record.is_deleted())), slot_id, &mut slot_adrs);
    ...
    fn rehash_truncate_separate_space(
        ...
        let copy = (free_spaces.clone(), space_version.clone(), page_slotids_2_adrs.clone());
        ...
        while let Some(mut largest_free_space) = free_spaces.pop() {
            ...
            while let Some(largest_used_space) = space_version.pop() {
                ...
                if largest_used_space.0.0 > largest_free_space.0 {
                    ...
                    assert!(largest_used_space.0.0 + largest_used_space.0.1 == largest_free_space.0, "{:?}, {:?}, free: {:?}, version: {:?}, slot_2_adr: {:?}", largest_used_space.0, largest_free_space, &copy.0, &copy.1, &copy.2);
                    let new_space = (largest_free_space.0 + largest_free_space.1 - largest_used_space.0.1, largest_used_space.0.1);
                    ...
                    self.write_bytes_overlapping(old_space.0 as usize..(old_space.0 + old_space.1) as usize, new_space.0 as usize);
                    
                    if version_idx == 0 {
                        // self.slot_offset(slot_id) + Slot::meta_offset_offset_in_slot()
                    } else {
                        let update_value_offset= versions[version_idx as usize - 1].0;
                        self.write_bytes(update_value_offset as usize, &u32::to_be_bytes(new_space.0));
                    };
        ...
        let new_slot_count = {
            ...
            for j in 0..slot_count {
                if let Some(versions) = page_slotids_2_adrs.get(&(j as u32)) {
                    // exist slot
                    slot_slice_mut[j].set_meta_offset(versions[0].0);
...
pub trait DoubleHashPageInterface {
...
pub trait TableDataPage: TableDataPageInterface {
    fn init(&mut self) {
    ...
    fn get(
        ...
        let slot = self.find_slot_by_kpk(key, pkey);
        slot
            .map(|slot| self.get_value(slot, ts))
            .unwrap_or(None)
            .map(|vec| Ok(vec))
    ...
    fn get_keys(
        ...
        Ok(slot_sli.iter()
            .filter(|slot| {
                self.check_slot_match_key(*slot, key)
            })
            .map(|slot| {
                self.get_value(slot, ts)
                    .map(|val| {
                        (self.get_pkey_by_slot(slot), val)
                    })
            })
            .filter_map(|res| res)
            .collect::<Vec<_>>())
    ...
    fn dbg_print_slots(&self) -> usize {
        ...
        for slot in slot_sli {
            log_warn!("{:?}", slot);
            SlotMeta::print_meta_of_slot(slot, self.read_bytes(slot.meta_offset() as usize, SlotMeta::space_need_from_slot(slot) as usize ));

            // insert versions to new page and collect free_space info
            let mut offset = get_first_record_offset_fn(slot);
            let mut record = self.next_version(offset);
            while let Some(commit_record) = record {
                log_warn!("[versionRecord] start offset: {offset}, space need: {}, {:?}", commit_record.space_need() ,commit_record);
    ...
    fn garbage_collect(&mut self, _safe_ts: Timestamp) {
        todo!()
    }
            ...
            if let Some(rec_off) = rec_off {
                ...
                if !commit_record.is_deleted() {
                    let (record_key, record_pkey) = {(
                        self.get_key_by_slot(slot),
                        self.get_pkey_by_slot(slot)
                    )};

                    if let Some(want_key) = key {
                        if record_key == want_key {
                            res.push( MvccEntry { key: record_key, pkey: record_pkey, value: commit_record.value, start_ts: commit_record.start_ts, end_ts: rec_end_ts } );    
                        }
                    } else {
                        res.push( MvccEntry { key: record_key, pkey: record_pkey, value: commit_record.value, start_ts: commit_record.start_ts, end_ts: rec_end_ts } );    
                    }
    ...
    fn scan_all_versions_all_keys(&self) -> Vec<MvccEntry> {
        ...
        for slot in slot_sli {
            ...
            while let Some(commit_record) = record {
                if !commit_record.is_deleted() {
                    res.push( MvccEntry { key: key.clone(), pkey: pkey.clone(), value: commit_record.value, start_ts: commit_record.start_ts, end_ts: prev_ts } );    
                }
                record = self.next_version(commit_record.next_offset);
    ...
    fn scan_delta_of_btw_ts(&self, small_ts: Timestamp, large_ts: Timestamp) -> Vec<DeltaEntry<Vec<u8>>> {
        let mut res = vec![];

        let slot_sli = self.get_slot_slice(0);
        for slot in slot_sli {
            let large_ts_record_res = self.find_latest_record_before_ts_by_slot(slot, large_ts);
            let (large_ts_rec_off, large_ts_rec_end_ts) = (large_ts_record_res.record_offset, large_ts_record_res.end_ts);
            ...
            let is_small_ts_not_exist = (small_ts_record_res.is_none() || small_ts_record_res.as_ref().unwrap().is_deleted());

            // no delta
            if large_ts_commit_record.is_deleted() 
                && is_small_ts_not_exist
            {
                continue;
            }
...
mod test_vec {
    ...
    fn test_small_key() {
        ...
        log_warn!("get with ts = 5: {:?}", <Page as TableDataPage>::get(&mut page, &[4,4,4], &[4,4,4], 5));
        log_warn!("get with ts = 4: {:?}", <Page as TableDataPage>::get(&mut page, &[4,4,4], &[4,4,4], 4));
        log_warn!("get with ts = 3: {:?}", <Page as TableDataPage>::get(&mut page, &[4,4,4], &[4,4,4], 3));
        log_warn!("get with ts = 2: {:?}", <Page as TableDataPage>::get(&mut page, &[4,4,4], &[4,4,4], 2));
        log_warn!("get with ts = 1: {:?}", <Page as TableDataPage>::get(&mut page, &[4,4,4], &[4,4,4], 1));
        log_warn!("get with ts = 0: {:?}", <Page as TableDataPage>::get(&mut page, &[4,4,4], &[4,4,4], 0));
    ...
    fn test_large_key() {
        ...
        log_warn!("get with ts = 5: {:?}", <Page as TableDataPage>::get(&mut page, &[4;10], &[4;10], 5));
        log_warn!("get with ts = 4: {:?}", <Page as TableDataPage>::get(&mut page, &[4;10], &[4;10], 4));
        log_warn!("get with ts = 3: {:?}", <Page as TableDataPage>::get(&mut page, &[4;10], &[4;10], 3));
        log_warn!("get with ts = 2: {:?}", <Page as TableDataPage>::get(&mut page, &[4;10], &[4;10], 2));
        log_warn!("get with ts = 1: {:?}", <Page as TableDataPage>::get(&mut page, &[4;10], &[4;10], 1));
        log_warn!("get with ts = 0: {:?}", <Page as TableDataPage>::get(&mut page, &[4;10], &[4;10], 0));
        page.dbg_print_slots();
    }
    ...
    fn test_mgd() {
        ...
        let (free_spaces, left_ids) = page.rehash_export_to_new_page(&mut new_page, fn1);
        log_warn!("free_space & left_ids{:?}", (free_spaces.clone().into_sorted_vec(), &left_ids));

        let (space_version_vec, slotids_2_versions) = page.rehash_collect_used_space(left_ids);
        log_warn!("left_space_index_vec & map: {:?}", (space_version_vec.clone().into_sorted_vec(), &slotids_2_versions));

        log_warn!("free_space:{}, slot_offset:{}, rec_start_offset: {}", page.free_space_without_compaction(), page.slot_end_offset(), page.rec_start_offset());
        

        page.rehash_truncate_separate_space(free_spaces, space_version_vec, slotids_2_versions);
