...
use super::{
    mvcc_hash_join_cuckoo_common::{
        BucketEntry, Buckets, ChainStatusEntry, CuckooAccessMethodError, SwapChainStatus,
        MAX_CUCKOO_ITERATE_COUNT,
    },
    mvcc_hash_join_cuckoo_page::MvccHashJoinCuckooPage,
};
    ...
    fn insert_inner(
        ...
        'insert: loop {
            ...
            let mut write_pages_map = {
                let mut entries_idxes = entry_index_with_op_sequential_chain
                    .iter()
                    .map(|elem| elem.0)
                    .collect::<Vec<_>>();
                let entries_idxes_sorted = {
                    entries_idxes.sort();
                    entries_idxes.dedup();
                    entries_idxes
                };
                let all_write_pages_map =
                    self.write_sequentially_all_pages(entries_idxes_sorted, &*buckets);

                // we need to check whether these are valid
                if !Self::check_ops_valid(
    ...
    */
            ...
            for bucket_page in check_bucket_pages_read.into_iter() {
                ...
                match MvccHashJoinCuckooPage::get_slot_id(&*bucket_page, key, pkey, ts) {
                    Ok(slot_id) => {
                        ...
                        match bucket_page.try_upgrade(true) {
                            Ok(mut upgraded_page) => {
                                match MvccHashJoinCuckooPage::delete_slot_at_id(
                                    &mut *upgraded_page,
                                    slot_id,
                                ) {
                                    Ok(old_res) => return Ok(old_res),
    ...
    pub fn get_swapped_entry_index_chain(
        ...
        'get_sorted_chain: loop {
            let check_page_frame_key = PageFrameKey::new_with_frame_id(
                ...
                buckets
                    .get_bucket_entry(check_entry_idx)
                    .frame_id
            ...
            if check_space_need <= MvccHashJoinCuckooPage::free_space_with_compaction(&*page) {
                // chain ends
                all_chain_entry_idx_with_status.push(ChainStatusEntry(
                    check_entry_idx,
                    SwapChainStatus::Insert(check_space_need),
                ));
            ...
            if iterate_cnt == MAX_CUCKOO_ITERATE_COUNT {
                return Err(CuckooAccessMethodError::CuckooIterateFailed(
                    buckets.get_bucket_num(),
                ));
            }
            ...
            'find_larger_and_swapptable_record: loop {
                match MvccHashJoinCuckooPage::check_larger_record(
                    &*page,
                    check_space_need,
                    check_slot_start_idx,
                ) {
                    ...
                    Some((swapped_slot_idx, swapped_key, swapped_pkey, swapped_val)) => {
                        ...
                        if let Some(a_second_entry_idx_of_slot) = buckets
                            .get_a_second_bucket_index(&swapped_key, check_slot_start_idx as usize)
                        {
                            /* new */
                            check_space_need = <Page as MvccHashJoinCuckooPage>::space_need(
                                &swapped_key,
                                &swapped_pkey,
                                &swapped_val,
                            );
                            all_chain_entry_idx_with_status.push(ChainStatusEntry(
                                /* old entry idx */ check_entry_idx,
                                SwapChainStatus::Swap((
                                    swapped_slot_idx,
                                    swapped_key.clone(),
                                    check_space_need,
                                )),
        ...
        for op in op_chain {
            ...
            match op.clone().1 {
                SwapChainStatus::Swap((swapped_slot_id, swapped_key, swapped_space_need)) => {
                    if <Page as MvccHashJoinCuckooPage>::check_slot_key_and_space_of_id(
                        &*write_page_guard,
                        swapped_slot_id,
                        &swapped_key,
                        swapped_space_need,
                    ...
                    match <Page as MvccHashJoinCuckooPage>::swap_record_at_slot_id(
                        ...
                        *want_swapping_slot_id,
                        ...
                        Ok((swapped_key, swapped_pkey, swapped_val, swapped_ts)) => {
                            // we can do that swap
                            let calc_space_need = <Page as MvccHashJoinCuckooPage>::space_need(
                                &swapped_key,
                                &swapped_pkey,
                                &swapped_val,
                            );
    ...
    pub fn rehash(&self, hash_size: u32) {
        ...
        for _ in 0..old_entry_num {
            buckets.buckets.push(BucketEntry {
                page_id: 0,
        ...
        for hashed_bucket_idx in 0..old_entry_num {
            ...
            buckets.buckets[(hashed_bucket_idx + old_entry_num) as usize] = BucketEntry {
                page_id: new_pid,
                frame_id: AtomicU32::new(new_fid),
            };
