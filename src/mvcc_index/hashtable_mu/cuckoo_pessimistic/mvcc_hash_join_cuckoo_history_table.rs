    ...
    pub fn get_swapped_entry_index_chain(
        ...
        'get_sorted_chain: loop {
            ...
            'find_larger_and_swapptable_record: loop {
                match MvccHashJoinCuckooHistoryPage::check_larger_record(
                    &*page,
                    check_space_need,
                    ...
                    Some((swapped_slot_idx, swapped_key, swapped_pkey, swapped_val)) => {
                        ...
                        {
                            /* new */
                            check_space_need = <Page as MvccHashJoinCuckooHistoryPage>::space_need(
                                &swapped_key,
                                &swapped_pkey,
                                &swapped_val,
                            );
        ...
        for op in op_chain {
            ...
            match op.clone().1 {
                SwapChainStatus::Swap((swapped_slot_id, swapped_key, swapped_space_need)) => {
                    if <Page as MvccHashJoinCuckooHistoryPage>::check_slot_key_and_space_of_id(
                        &*write_page_guard,
                        swapped_slot_id,
                        &swapped_key,
                        swapped_space_need,
                    ...
                    match <Page as MvccHashJoinCuckooHistoryPage>::swap_record_at_slot_id(
                        ...
                        *want_swapping_slot_id,
                        ...
                        Ok((
                            ...
                            // we can do that swap
                            let calc_space_need =
                                <Page as MvccHashJoinCuckooHistoryPage>::space_need(
                                    &swapped_key,
                                    &swapped_pkey,
                                    &swapped_val,
                                );
